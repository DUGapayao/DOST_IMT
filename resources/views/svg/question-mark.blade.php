<svg width="40px" height="40px" viewBox="-4.8 -4.8 57.60 57.60" xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF">
    <!-- Question mark icon -->
    <text x="0" y="-10" font-size="16" fill="black">?</text>
    
    <g id="SVGRepo_bgCarrier" stroke-width="0" opacity="0.5">
      <rect x="-4.8" y="-4.8" width="57.60" height="57.60" rx="28.8" fill="#D9D9D9" stroke-width="0"></rect>
    </g>
    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
    <g id="SVGRepo_iconCarrier">
      <g id="Layer_2" data-name="Layer 2">
        <g id="invisible_box" data-name="invisible box">
          <rect width="48" height="48" fill="none"></rect>
        </g>
        <g id="Layer_7" data-name="Layer 7">
          <g>
            <path d="M24.3,6A11.2,11.2,0,0,0,16,9.3a11,11,0,0,0-3.5,8.2,2.5,2.5,0,0,0,5,0,6.5,6.5,0,0,1,2-4.7A6.2,6.2,0,0,1,24.2,11a6.5,6.5,0,0,1,1,12.9,4.4,4.4,0,0,0-3.7,4.4v3.2a2.5,2.5,0,0,0,5,0V28.7a11.6,11.6,0,0,0,9-11.5A11.7,11.7,0,0,0,24.3,6Z"></path>
            <circle cx="24" cy="39.5" r="2.5"></circle>
          </g>
        </g>
      </g>
    </g>
  </svg>